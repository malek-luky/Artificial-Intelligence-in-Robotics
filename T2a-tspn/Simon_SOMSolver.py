...
class Neuron:
    ...
    def update_goal_position(self, goal, radius):
        ...
        vec = (self.position - goal)
        vs = np.sqrt(np.sum(vec**2))
        if vs > radius:
            vec /= vs
            self.goal_position = goal + vec*radius
        else:
    ...
    def select_winner(self, goal_idx):
        ...
        for idx, neuron in enumerate(self.neurons):
            if neuron.goal_idx == None:
                ...
                dist = (neuron.position[0]-act_goal_x)**2 + (neuron.position[1]-act_goal_y)**2
                if dist < min_dist:
                    best_idx = idx
                    min_dist = dist
    ...
    def learn_epoch(self, learning_epoch):
        ...
        for goal_idx in order:
            ...
            for i, d in enumerate(distances):
                n_idx = winner_idx+d
                act_beta = betas[i]
                neigh = self.neurons[n_idx % self.M]
                neigh.adapt_neuron(act_beta, alternate_goal)
