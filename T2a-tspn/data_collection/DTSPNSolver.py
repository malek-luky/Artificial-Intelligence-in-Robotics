...
def plan_tour_noon_bean(goals, sensing_radius, turning_radius):
    # Disclaimer: Inspired by Simon Zvara, after 20h of debugging I gave up and inspired by his code
    """
    ...
    for point1 in range(number_of_points):
        for point2 in range(number_of_points):
            ...
            for sample1 in range(samples_per_point):
                for sample2 in range(samples_per_point):
    ...
    const_M = np.max(distances2) #TODO: Super important to have it as max, speeds up the computatinos
    # from each point check all other points
    for point1 in range(number_of_points):
        for point2 in range(number_of_points):
            #now from each sample check all otehr samples
            for sample1 in range(samples_per_point):
                for sample2 in range(samples_per_point): 
                    # create the circles of zeros and inf for the neighbors
                    if point1 == point2:
                    ...
                    # switch the edges so we end at the same point
                    else:
    ...
    for i in range(len(sequence_TSP)):
        ...
        if point != last_point and len(sequence) != N: #append only for the last point
