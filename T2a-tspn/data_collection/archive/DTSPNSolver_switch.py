...
def plan_tour_decoupled(goals, sensing_radius, turning_radius):
    ...
    # START PF MY CODE
    samples = [samples[x] for x in sequence] # reorder based on the correct way
    samples_per_point = len(samples[0])
    number_of_points = len(samples)

    # CHECK ALL POSSIBLE ROUTES
    length_matrix = np.zeros([samples_per_point,number_of_points]) #matrix of length for each point
    previos_node_matrix = np.zeros([samples_per_point,number_of_points]) #matrix of previous node for each point
    for col in range(number_of_points):
        print("COL",col)
        for row in range (samples_per_point):
            print("ROW",row)
            for next_row in range(samples_per_point):
                # get the distance between point [row][col] and [next_row][col+1]
                distance = length_matrix[next_row][col] + dubins.shortest_path(pose_to_se2(samples[col][row]), pose_to_se2(samples[(col+1)%number_of_points][next_row]), turning_radius).path_length()
                print("DISTANCE",distance)
                if length_matrix[next_row][(col+1)%number_of_points]==0 or distance < length_matrix[next_row][(col+1)%number_of_points]:
                    length_matrix[next_row][(col+1)%number_of_points] = distance
                    previos_node_matrix[next_row][(col+1)%number_of_points] = row
    print(length_matrix)
    print(previos_node_matrix)
    
    sleep(100)
    # print(number_of_points * [list(range(samples_per_point))])
    # possible_permutations = list(product(number_of_points * [list(range(samples_per_point))]))
    ...
    configurations = []
    for idx in range(N):
        configurations.append (sample[sequence[idx]][selected_samples[idx]])
    # START

    path, path_length = configurations_to_path(configurations, turning_radius)
    ...
    return best_sample, best_path_length
    #END
