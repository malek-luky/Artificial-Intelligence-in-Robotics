    ...
    for row in range (number_of_points):
        for col in range(samples_per_point):
            for next_col in range(samples_per_point):
                print(row,col,next_col)
                # get the distance between point [row][col] and [next_col][col+1]
                distance = length_matrix[row][next_col]+ dubins.shortest_path(pose_to_se2(samples[row][col]), pose_to_se2(samples[(row+1)%number_of_points][next_col]), turning_radius).path_length()
