...
class Neuron:
    ...
    def update_goal_position(self, goal, radius):
        """
        The closest point on the boundary of the neighborhood region is selected to shorten the final tour.
        If the selected neuron is already in the region, the alternate goal is set directly to its position.
        """
        distance = np.sqrt(np.sum((goal - self.position) ** 2))
        if distance < radius:
            self.goal_position = self.position
        else:
            self.goal_position = goal + radius * (self.position-goal) / distance
...
class SOMSolver:
    ...
    def select_winner(self, goal_idx):
        ...
        for idx, neuron in enumerate(self.neurons):
            if neuron.goal_idx == None:
                ...
                """
                Select the closest non-inhibited neuron to the target. (best_idx)
                Inhibit the selected neuron such that it cannot be selected twice.
                (use squared distance for a faster computation)
                """
                distance = np.sum((neuron.position - np.array([act_goal_x, act_goal_y])) ** 2)
    ...
    def learn_epoch(self, learning_epoch):
        ...
        for goal_idx in order:
            ...
            Update goal positions in the neighborhoods
            If only the selected neuron is adapted, the final tour does not converge the to final solution.
            Thus a neighborhood of several neurons is also adapted towards the goal.
            Use 20% of neurons on both side and compute the learning parameter by the provided neighborhood function.
            Notice the distance is discrete and measured as number of neurons from the selected neuron.
            """
            for i, d in enumerate(distances):
                idx = winner_idx + d
                act_beta = betas[i]
                neighbors = self.neurons[idx % self.M]
                neighbors.adapt_neuron(act_beta, alternate_goal)
            # TODO - End of my code
