...
def plan_tour_decoupled(goals, sensing_radius, turning_radius):
    ...
    length_matrix = 100*np.ones([number_of_points,samples_per_point]) #matrix of length for each point
                ...
                for next_col in range(samples_per_point):
                    ...
                    if distance < length_matrix[(row+1)][next_col]:
                        ...
                        if row==number_of_points-2:
                            last_col = row+1
                            distance = length_matrix[last_col][next_col] + dubins.shortest_path(pose_to_se2(samples[row+1][next_col]), pose_to_se2(samples[0][start]), turning_radius).path_length()
