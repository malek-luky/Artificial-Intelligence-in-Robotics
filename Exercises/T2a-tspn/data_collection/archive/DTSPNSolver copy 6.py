...
def plan_tour_decoupled(goals, sensing_radius, turning_radius):
    ...
    length_matrix = 999999*np.ones([number_of_points,samples_per_point]) #matrix of length for each point
    ...
    for start in range(samples_per_point):
        ...
        for col in range(samples_per_point):
            distance = dubins.shortest_path(pose_to_se2(samples[0][start]), pose_to_se2(samples[1][col]), turning_radius).path_length()
        ...
        for row in range (1,number_of_points-1):
            for col in range(samples_per_point):
                for next_col in range(samples_per_point):
                    ...
                    distance = length_matrix[row][col]+ dubins.shortest_path(pose_to_se2(samples[row][col]), pose_to_se2(samples[(row+1)][next_col]), turning_radius).path_length()
                    if row==number_of_points-2:
                        last_col = row+1
                        distance_start = distance + dubins.shortest_path(pose_to_se2(samples[row+1][next_col]), pose_to_se2(samples[0][start]), turning_radius).path_length()
                        if distance_start < length_matrix[0][start]:
                            length_matrix[0][start] = distance_start
                        ...
                        if distance < length_matrix[(row+1)][next_col]:
                            length_matrix[(row+1)][next_col] = distance
                            previos_node_matrix[(row+1)][next_col] = col
                            # add the distance to the start point
