...
def plan_tour_decoupled(goals, sensing_radius, turning_radius):
    ...
    length_matrix = 999*np.ones([number_of_points,samples_per_point]) #matrix of length for each point
    ...
    for start in range(samples_per_point):
        ...
        for col in range(samples_per_point):
            ...
            if length_matrix[1][col]==0 or distance <= length_matrix[1][col]:
                print("distance: ", distance, "start", start)
            ...
            for col in range(samples_per_point):
                for next_col in range(samples_per_point):
                    ...
                    if row==number_of_points-2:
                        last_col = row+1
                        distance_start = distance + dubins.shortest_path(se2[row+1][next_col], se2[0][start], turning_radius).path_length()
                        if length_matrix[0][start]==0 or distance_start <= length_matrix[0][start]:
